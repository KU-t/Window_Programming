...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		{
		case ID_MENU_START:      
			answer = MessageBox(hWnd, "START", "Agra.io-" , MB_OKCANCEL | MB_ICONQUESTION);
			if (answer == IDOK) {
				SetTimer(hWnd, 1, 500, NULL);	// 먹이생성
				SetTimer(hWnd, 2, 50, NULL);	// ME[0]움직임
				SetTimer(hWnd, 3, 3000, NULL);	// 적생성
		...
		case ID_MENU_PAUSE:
			answer = MessageBox(hWnd, "PAUSE", "Agra.io-", MB_OKCANCEL | MB_ICONQUESTION);
			if (answer == IDOK) {
				KillTimer(hWnd, 1);
				KillTimer(hWnd, 2);
				KillTimer(hWnd, 3);
